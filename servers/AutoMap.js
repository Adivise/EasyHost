...
client.connect().then(async () => {
	...
	lobby.channel.on("message", async (message) => {
		...
		if (command === "start") {
			...
			channel.sendMessage(`*Rules* | Star Rating: ${AutoMap.min_star}* - ${AutoMap.max_star}* | Mode: Standard | Mods: ${AutoMap.mods.join(", ")} | FreeMod: ${AutoMap.freemod ? "Allowed" : "Not Allowed"}`);
